...
def getStatusInstall(ovhclient, module):
    if module.params["name"]:
        ...
        for i in range(1, int(module.params["max_retry"])):
            ...
            try:
                tasklist = ovhclient.get(
                    "/dedicated/server/%s/task" % module.params["name"],
                    function="reinstallServer",
                )
                result = ovhclient.get(
                    "/dedicated/server/%s/task/%s"
                    % (module.params["name"], max(tasklist))
                )
            ...
            if "done" not in result["status"]:
                progress_status = ovhclient.get(
                    "/dedicated/server/%s/install/status" % module.params["name"]
                )
                if (
                ...
                module.exit_json(
                    changed=False, msg="%s: %s" % (result["status"], message)
                )
        module.fail_json(
            changed=False,
            msg="Max wait time reached, about %i x %i seconds"
            % (i, int(module.params["max_retry"])),
        )
    else:
        module.fail_json(
            changed=False,
            msg="Please provide 'ns' server name from wich installation status will be check",
        )
...
def launchInstall(ovhclient, module):
    if (
        module.params["name"]
        and module.params["hostname"]
        and module.params["template"]
    ):
        try:
            compatible_templates = ovhclient.get(
                "/dedicated/server/%s/install/compatibleTemplates"
                % module.params["name"]
            )
            compatible_templates = set(
                [
                    ...
                    for template in compatible_templates[template_type]
                ]
            )
            if module.params["template"] not in compatible_templates:
                module.fail_json(
                    changed=False,
                    msg="%s doesn't exist in compatibles templates"
        ...
        if module.check_mode:
            module.exit_json(
                changed=True,
                msg="Installation in progress on %s ! - (dry run mode)"
                % module.params["name"],
            )
        details = {
            "details": {"language": "en", "customHostname": module.params["hostname"]},
        ...
        if module.params.get("ssh_key_name", None):
            try:
                ...
                if module.params["ssh_key_name"] not in result:
                    module.fail_json(
                        changed=False,
                        msg="%s doesn't exist in public SSH keys"
                        % module.params["ssh_key_name"],
                    )
                else:
        ...
        if not module.params["name"]:
            module.fail_json(
                changed=False, msg="Please give the service's name you want to install"
            )
        ...
        if not module.params["hostname"]:
            module.fail_json(
                changed=False, msg="Please give a hostname for your installation"
            )
...
def changeMonitoring(ovhclient, module):
    if module.params["name"] and module.params["state"]:
        if module.params["state"] == "present":
            ...
            module.fail_json(
                changed=False,
                msg="State %s does not match 'present' or 'absent'"
                % module.params["state"],
            )
        ...
        if module.check_mode:
            module.exit_json(
                changed=True,
                msg="Monitoring %s on %s - (dry run mode)"
        ...
        for i in range(1, int(module.params["max_retry"])):
            ...
            if server_state["monitoring"] != shouldbe:
                try:
                    ovhclient.put(
                        "/dedicated/server/%s" % module.params["name"],
                        monitoring=shouldbe,
                    )
                except APIError as apiError:
                    module.fail_json(
                ...
                if shouldbe:
                    module.exit_json(
                        changed=True,
                        msg="Monitoring activated on %s after %i time(s)"
                        % (module.params["name"], i),
                    )
                else:
                    module.exit_json(
                        changed=True,
                        msg="Monitoring deactivated on %s after %i time(s)"
                        % (module.params["name"], i),
                    )
            time.sleep(float(module.params["sleep"]))
        ...
        if not module.params["name"]:
            module.fail_json(
                changed=False, msg="Please give a name to change monitoring state"
            )
        if not module.params["state"]:
            module.fail_json(
                changed=False, msg="Please give a state for your monitoring"
            )
...
def terminateServer(ovhclient, module):
    if module.params["name"]:
        if module.check_mode:
            module.exit_json(
                changed=True,
                msg="Terminate %s is done, please confirm via the email sent - (dry run mode)"
                % module.params["name"],
            )
        try:
            ...
            module.exit_json(
                changed=True,
                msg="Terminate %s is done, please confirm via the email sent"
                % module.params["name"],
            )
        except APIError as apiError:
            module.fail_json(
    ...
    if module.params["domain"] and module.params["ip"]:
        ...
        if result["reverse"] != fqdn:
            if module.check_mode:
                module.exit_json(
                    changed=True,
                    msg="Reverse %s to %s succesfully set ! - (dry run mode)"
                    % (module.params["ip"], fqdn),
                )
            try:
                ovhclient.post(
                    "/ip/%s/reverse" % module.params["ip"],
                    ipReverse=module.params["ip"],
                    reverse=fqdn,
                )
                module.exit_json(
                    changed=True,
                    msg="Reverse %s to %s succesfully set !"
                    % (module.params["ip"], fqdn),
                )
            except APIError as apiError:
                module.fail_json(
        ...
        if not module.params["domain"]:
            module.fail_json(
                changed=False, msg="Please give a domain to add your target"
            )
...
def changeDNS(ovhclient, module):
    ...
    if module.params["name"] == "refresh":
        if module.check_mode:
            module.exit_json(
                changed=True,
                msg="Domain %s succesfully refreshed ! - (dry run mode)"
...
def changeVRACK(ovhclient, module):
    if module.params["vrack"]:
        if module.check_mode:
            module.exit_json(
                changed=True,
                msg="%s succesfully %s on %s - (dry run mode)"
                % (
                    module.params["name"],
                    module.params["state"],
                    module.params["vrack"],
        ...
        if module.params["state"] == "present":
            ...
            if len(result):
                try:
                    is_already_registered = ovhclient.get(
                        "/vrack/%s/dedicatedServerInterfaceDetails"
                        % module.params["vrack"]
                    )
                except APIError as apiError:
                    module.fail_json(
                ...
                for new_server in is_already_registered:
                    if new_server["dedicatedServer"] == module.params["name"]:
                        module.exit_json(
                            changed=False,
                            msg="%s is already registered on %s"
                            % (module.params["name"], module.params["vrack"]),
                        )
                try:
                    serverInterface = "".join(result)
                    result2 = ovhclient.post(
                        "/vrack/%s/dedicatedServerInterface" % module.params["vrack"],
            ...
            # Old generation
                ...
                try:
                    is_already_registered = ovhclient.get(
                        "/vrack/%s/dedicatedServer" % module.params["vrack"]
                    )
                except APIError as apiError:
                    module.fail_json(
                        changed=False,
            ...
            try:
                result_new = ovhclient.get(
                    "/vrack/%s/dedicatedServerInterfaceDetails" % module.params["vrack"]
                )
                result_old = ovhclient.get(
                    "/vrack/%s/dedicatedServer" % module.params["vrack"]
                )
            except APIError as apiError:
                module.fail_json(
            ...
            for new_server in result_new:
                if new_server["dedicatedServer"] == module.params["name"]:
                    try:
                        result = ovhclient.delete(
                            "/vrack/%s/dedicatedServerInterface/%s"
                            % (
                                module.params["vrack"],
                                new_server["dedicatedServerInterface"],
                            )
                        )
                        module.exit_json(changed=True, contents=result)
                    except APIError as apiError:
            ...
            for old_server in result_old:
                if old_server == module.params["name"]:
                    try:
                        result = ovhclient.delete(
                            "/vrack/%s/dedicatedServer/%s"
                            % (module.params["vrack"], module.params["name"])
                        )
                        module.exit_json(changed=True, contents=result)
                    except APIError as apiError:
                        module.fail_json(
            ...
            module.exit_json(
                changed=False,
                msg="No %s in %s" % (module.params["name"], module.params["vrack"]),
            )
        else:
            module.exit_json(
                changed=False, msg="Vrack service only uses present/absent state"
            )
    else:
        module.fail_json(
            changed=False, msg="Please give a vrack name to add/remove your server"
        )
...
def generateTemplate(ovhclient, module):
    if module.params["template"]:
        if module.check_mode:
            module.exit_json(
                changed=True,
                msg="%s succesfully %s on ovh API - (dry run mode)"
                % (module.params["template"], module.params["state"]),
            )
        ...
        if module.params["state"] == "present":
            ...
            Templates = {
                "customization": {
                    ...
                    "postInstallationScriptReturn": conf[
            ...
            module.exit_json(
                changed=True,
                msg="Template %s succesfully created" % conf["templateName"],
            )
        elif module.params["state"] == "absent":
            try:
            ...
            module.exit_json(
                changed=True,
                msg="Template %s succesfully deleted" % conf["templateName"],
            )
        else:
            module.fail_json(
                changed=False,
                msg="State %s not supported. Only present/absent"
                % module.params["state"],
            )
    else:
...
def changeBootDedicated(ovhclient, module):
    ...
    if module.check_mode:
        module.exit_json(
            changed=True,
            msg="%s is now set to boot on %s. Reboot in progress... - (dry run mode)"
            % (module.params["name"], module.params["boot"]),
        )
    try:
    ...
    if bootid[module.params["boot"]] != check["bootId"]:
        try:
            ...
            module.exit_json(
                changed=True,
                msg="%s is now set to boot on %s. Reboot in progress..."
                % (module.params["name"], module.params["boot"]),
            )
        except APIError as apiError:
            module.fail_json(
        ...
        if (
            module.params["force_reboot"] == "yes"
            or module.params["force_reboot"] == "true"
        ):
            try:
                ovhclient.post("/dedicated/server/%s/reboot" % module.params["name"])
            except APIError as apiError:
        ...
        module.exit_json(
            changed=False,
            msg="%s already configured for boot on %s"
            % (module.params["name"], module.params["boot"]),
        )
...
def getMac(ovhclient, module):
    result = ovhclient.get(
        ...
        % (module.params["name"], module.params["link_type"])
    )
...
def main():
    ...
    if module.params["service"] == "dns":
        ...
        if module.params["name"] == "dedicated":
            ...
            module.fail_json(
                changed=False,
                msg="%s not supported for 'list' service" % module.params["name"],
            )
